...
Amplify ETF holdings to Watchlist. A single-file client-side tool that reads the generated `./api/amplify` static feed (Amplify ETFs Firestore data feed, Yahoo Finance history) into a searchable ETF/category catalog with per-fund tabs, watchlist aggregation, ticker copy and CSV/TXT export — the same look, feel, columns and business logic as the sibling applications.
...
The **Update Amplify ETF data** GitHub Actions workflow exposes the same settings as manual inputs. All supplied filters use **AND** logic.

### Data sources

| Block | Source |
| --- | --- |
| Catalog (all US Amplify ETFs) | `https://amplifyetfs.com/` → Firestore `https://firestore.googleapis.com/v1/projects/amplify-etfs/databases/(default)/documents/` (catalog) |
| Holdings per fund | Firestore per-fund holdings collections |
| Daily history, distributions | Yahoo Finance chart API (when needed) |
| Fallback | Previously published `api/amplify/index.json` |
...
| --- | --: | --- |
| `MAX_FETCHES` | all | Batch size: with a positive value the updater continues after the committed cursor in `api/amplify/update-state.json`; empty or `0` is a full pass — every fund is refreshed in one run. |
