...
const el = {
  ...
  tickerCount: byId<HTMLSpanElement>('ticker-count'),
  subtitle: byId<HTMLSpanElement>('app-subtitle'),
  searchInput: byId<HTMLInputElement>('search-input'),
  tabsBar: byId<HTMLDivElement>('tabs-bar'),
  copyBtn: byId<HTMLButtonElement>('copy-btn'),
  exportCsvBtn: byId<HTMLButtonElement>('export-csv-btn'),
  exportTxtBtn: byId<HTMLButtonElement>('export-txt-btn'),
  resetBtn: byId<HTMLButtonElement>('reset-btn'),
  statusLine: byId<HTMLDivElement>('status-line'),
  tableHead: byId<HTMLTableSectionElement>('table-head'),
...
async function loadData(): Promise<void> {
  try {
    ...
    const data = await response.json() as DataApi;
...
function renderFundsTable(): void {
  ...
  el.tableBody.querySelectorAll<HTMLTableRowElement>('tr[data-ticker]').forEach(row => {
...
function bindSortHeaders(): void {
  el.tableHead.querySelectorAll<HTMLButtonElement>('button[data-sort]').forEach(button => {
    button.addEventListener('click', () => {
