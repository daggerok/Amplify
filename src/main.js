...
var ACTIVE_FUND_KEY = "amplify-active-fund";
var DETAIL_TABS = [
...
function renderDetailTable(tab) {
  const activeFund = getActiveFund();
...
function renderOverviewTable(fund, detail) {
  ...
  setDetailStatus(fund, `Overview \xB7 ${rows.length} metrics`);
  el.tickerCount.textContent = `${fund.ticker}`;
}
function renderSelectedFundHoldings(fund) {
  ...
  setDetailStatus(fund, `Holdings as of ${state.holdings[fund.ticker]?.asOfDate || "latest"} \xB7 ${rows.length} rows`);
  el.tickerCount.textContent = `${rows.length} holdings`;
}
function renderPerformanceTable(fund, detail) {
  ...
  setDetailStatus(fund, `Performance and total returns \xB7 ${rows.length} rows`);
  el.tickerCount.textContent = `${rows.length} returns`;
}
function renderAllocationsTable(fund, detail) {
  ...
  setDetailStatus(fund, `Allocations as of ${detail.allocations?.asOfDate || "latest"} \xB7 ${rows.length} rows`);
  el.tickerCount.textContent = `${rows.length} allocations`;
}
function renderDistributionsTable(fund, detail) {
  ...
  setDetailStatus(fund, `Distributions \xB7 ${rows.length} rows`);
  el.tickerCount.textContent = `${rows.length} distributions`;
}
function renderYieldsTable(fund, detail) {
  ...
  setDetailStatus(fund, `Yields as of ${detail.yields?.asOfDate || "latest"} \xB7 ${rows.length} rows`);
  el.tickerCount.textContent = `${rows.length} yields`;
}
function renderPriceTable(fund, detail) {
  ...
  setDetailStatus(fund, `NAV / market price as of ${detail.daily?.asOfDate || "latest"} \xB7 ${rows.length} rows`);
  el.tickerCount.textContent = `${rows.length} metrics`;
}
function renderSimpleRows(rows, columns, emptyText) {
  if (!rows.length) {
...
function renderControls() {
  ...
  const active = state.activeFundTicker ? ` \xB7 active ${state.activeFundTicker}` : "";
...
function getOverviewRows(fund, detail) {
...
function metricRow(section, metric, value) {
  const text = value === void 0 || value === null || value === "" ? "\u2014" : String(value);
  return {
    section,
    metric,
    value: text,
...
function getFundHoldingRows(ticker) {
...
function getPerformanceRows(detail) {
  const docs = [
  ...
  docs.forEach(({ period, doc }) => {
    (doc?.returns || []).forEach((item) => {
      ...
      Object.entries(returns).forEach(([key, value]) => {
        row[key] = numberOrNull(value) ?? value;
      });
      rows.push(row);
    });
  });
...
function getAllocationRows(detail) {
  const asOfDate = detail.allocations?.asOfDate || "";
  return (detail.allocations?.allocations || []).flatMap(
    (allocation) => (allocation.dimvalues || []).map((value) => ({
...
function getDistributionRows(detail) {
...
function exportCsv() {
  ...
  if (isDetailTab(state.activeTab)) {
    const tab = detailTabKey(state.activeTab);
    const rows2 = getCurrentVisibleRows();
    const headers2 = Object.keys(rows2[0] || {}).filter((key) => key !== "searchIndex");
    downloadText(toCsv([headers2, ...rows2.map((row) => headers2.map((header) => cellToString(row[header])))]), exportFileName(`${getActiveFund()?.ticker || "ETF"}_${tab}`, "csv"), "text/csv;charset=utf-8;");
    return;
  }
...
function exportTxt() {
  let values;
  if (state.activeTab === "watchlist") {
    values = getVisibleWatchlistRows().map((row) => row.symbol).sort((a, b) => a.localeCompare(b, void 0, { numeric: true }));
  } else if (isDetailTab(state.activeTab)) {
    values = getCurrentVisibleRows().map((row) => Object.entries(row).filter(([key]) => key !== "searchIndex").map(([key, value]) => `${key}: ${cellToString(value)}`).join(" | "));
  } else {
...
function setDetailStatus(fund, message) {
...
function isDetailTab(tab) {
  return tab.startsWith("detail:");
}
function detailTabKey(tab) {
  return tab.replace("detail:", "") || "overview";
}
...
function returnLabel(key) {
  return { SI_Cum: "SI Cum.", SI_Ann: "SI Ann." }[key] || key;
}
...
function formatMaybeMoney(value, decimals) {
  const parsed = numberOrNull(value);
  if (parsed === null) return "\u2014";
...
function formatDistributionAmount(value) {
  const parsed = numberOrNull(value);
  if (parsed === null) return "\u2014";
